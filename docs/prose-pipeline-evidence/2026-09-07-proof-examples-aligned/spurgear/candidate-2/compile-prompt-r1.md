Compile the specification for `spurgear` into a step list and a runnable proof. Work in the repo
worktree. Write the step list to `.tmp/spurgear.steps.md`, and the proof, as one or more Go files,
to `.tmp/spurgear-proof/`.

**Read, in full, only these:** `spec/spurgear/instructions.md`, `spec/spurgear/fusion.md` if it
exists, `spec/spurgear/contract.json` if it exists, every Markdown document the prose sources
...
**Do not read** `lib/geargen/spurgear.py`, any other gear's implementation, or a previous
...
before any other section, written as the committed paths `proof/spurgear/<file>.go` even though you
are writing the files themselves to `.tmp/spurgear-proof/`, since the step list ships next to the
...
Include `proof/spurgear/zz_registrations_test.go`, the generated registration file, among the
paths.
...
**The proof is a Go test** in package `spurgear_test`, spread over as many files as the split
...
turns these annotations into `proof/spurgear/zz_registrations_test.go`; never write that file,
