Write the Fusion 360 add-in implementation for `spurgear` by following its compiled step list. Work
in the repo worktree. Write `.tmp/spurgear.generated.py`, then stop drafting and report what you
...
**Read, in this order:** `spec/spurgear/steps.md`, including its complete generated
`## Compilation contract` section when present, which carries every manifest rule the output must
satisfy, then work through the timeline steps in order; `proof/spurgear/`, the checked geometry,
which steps tagged `[GO]` tell you to
transliterate literally rather than re-derive; `.tmp/spurgear.playbook-extract.md`, the generated
...
`.tmp/spurgear.emitter-interfaces.md`, whose typed entry-point signatures final code must implement
...
**Do not read** `lib/geargen/spurgear.py`, `spec/spurgear/instructions.md`, `spec/spurgear/fusion.md`,
