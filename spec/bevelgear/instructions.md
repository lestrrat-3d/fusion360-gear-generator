...
- the **trace arc** — a 3-point arc through `tanW(toe2d)`, `tanW(R_mean, 0)` (the mean point on the cone element), `tanW(heel2d)`, with its **centre coincident to the cutter circle's centre** and a **radius dimension = `r_c`**, so it is the genuine cutter circle and not a look-alike spline. ⚠️ **The radius dimension's text point must be ON the arc — e.g. the mean point `tanW(R_mean, 0)` — NEVER the arc's centre.** `addRadialDimension(arc, textPoint)` rejects a text point at the centre with `RuntimeError: 3 : 一部の入力引数が無効です` ("some input arguments are invalid") — there is no radial direction at the centre. The cutter circle's diameter-dimension text point must likewise be **off-centre**, a point on the circle such as `tanW(Cx + r_c, Cy)`.
