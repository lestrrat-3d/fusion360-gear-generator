...
Shaft Angle: User-supplied angle in degrees, **at least 30°** and **at most the Maximum Shaft Angle** below. Default 90° (perpendicular shafts — the classic bevel pair). The input is a `deg` Fusion expression (e.g. `60 deg`); convert to degrees before the range check (read-back units: see "Reading the raw numbers", `[PB-EVAL-EXPRESSION]`). 30° itself is reachable, but only because the Maximum Base Height below caps the default base height there; without that cap the default pair folds its own profile at exactly 30°.
...
so that `acos` is a hard singularity and the range check must reject a Shaft Angle **at or above** `degrees(acos(-smaller / larger))`, naming the computed limit in the message. A 31/17 pair gives `acos(-17/31) = 123.24°`; the flat 150° this spec used to promise was never reachable for it. Equal tooth counts give `acos(-1) = 180°`, which is no constraint at all.

**A second, lower ceiling of 140° applies to every pair**, and it is not a cone-angle argument — it is measured. The §2 lattice becomes ill-conditioned as the Shaft Angle rises, and the sketch engine refuses a system below its `4e-5` conditioning floor. For the **default equal 31/31 pair**: 135° and 140° pass, 142° comes back at `3.96e-5` and is refused, 144° passes again, and 149° (`2.82e-5`) and 150° (`2.49e-5`) are firmly under. The measurement wobbles either side of the floor from about 141° upward rather than crossing it cleanly, so 140° is taken as the last angle that holds with margin. An equal 8-tooth pair is already refused at 150° (`8.2e-6`), so a smaller tooth count makes this worse, never better.

**The Maximum Shaft Angle is therefore the smaller of 140° and `degrees(acos(-smaller / larger))`**, and the cone-angle half is exclusive while the 140° half is inclusive.
...
`1.25 * Module` is the dedendum, so `r - 1.25 * Module * cos γ` is the perpendicular distance from the shaft axis out to the dedendum point C (pinion) or D (driving) at the heel, and multiplying by `tan γ` gives the along-shaft rise at which the heel edge C->H (resp. D->J) reaches that axis. Past it the hexagonal frustum profile has crossed its own axis of revolution, which fails the revolve with `ASM_WIRE_X_AXIS` exactly as an over-large Face Width does (`[PB-REVOLVE]`). The `0.95` factor is the one the Maximum Face Width uses, and for the same reason: a heel edge that merely touches the axis is already degenerate.
...
Worked case, because it is the default configuration and it used to fail: Module 1, Driving 31, Pinion 31, Shaft Angle 30°. Each `γ` is 15°, so the bound is `0.95 * (15.5 - 1.25*cos 15°) * tan 15° = 3.638 mm`, while the driving fallback `Module * Driving Gear Teeth Number / 8` resolves to `3.875 mm`. Uncapped, the shipped default folded its own profile at the lowest Shaft Angle the spec claimed to support; the raw crossing point is 3.830 mm, so the fallback overshot it by barely more than 1%. With the cap that configuration builds, capped to 3.638 mm. The fallback stops being capped, for this pair, from about Shaft Angle 31.85° upward.
...
(the constant is `2 * (1.05 * 1.25 / 0.95 + 1.25)`). Check it per gear with that gear's own `γ`, and reject below it naming the computed floor. At Shaft Angle 90° the floor is 3.72, i.e. **4 teeth** — measured: with both bounds applied an equal 4-tooth pair solves and a 3-tooth pair still fails on the heel edge, because its Maximum Base Height has fallen below its Minimum. The blanket `teeth >= 3` check admits that 3-tooth pair, so keep `teeth >= 3` as the absolute floor and apply this computed floor on top of it.

⚠️ **A configuration can satisfy every bound above and still be refused as near-singular.** The §2 lattice's conditioning degrades as the Shaft Angle rises and the tooth count falls, and the sketch engine refuses a system whose conditioning drops below its `4e-5` trust floor. The 140° ceiling in the Maximum Shaft Angle is where that was measured for the default 31/31 pair, but the two effects compound: a small tooth count reaches the floor at a *lower* Shaft Angle than a large one, so an angle inside the range for 31 teeth is not guaranteed for 8. There is no closed form for this, so it is not a validation rule — it is a limit the proof discovers, and the proof's case table is where the known-bad combinations belong. Treat a near-singular report as a real refusal, never as a tolerance to loosen.
...
Connect point G and H with a line. Constrain end points of line accordingly with coincidence constraints. **Add no perpendicular constraint between E->G and H->G.** Earlier revisions of this spec asked for one; with E->G already parallel to A->E and C->H already collinear with Apex2->C, that perpendicular fixes the same freedom the pinion base-height offset dimension below fixes, and the two together over-determine the sketch. Measured: adding it and its driving-side twin makes the lattice report **overconstrained at DOF 0 with 2 redundant constraints, and the engine names the two base-height offsets as the redundant pair** — so the perpendicular would silently take over the base height and the dimension that is supposed to set it would stop meaning anything (`[PB-NO-OVERCONSTRAIN]`).
...
Connect point I and J with a line. Constrain end points of line accordingly with coincidence constraints. **Add no perpendicular constraint between F->I and J->I** — the driving-side twin of the G->H case just above, and redundant for the same reason against the driving base-height offset.
