...
Create line M->N. **Seed it near its solved position** (`[PB-SEED-NEAR]`): seed M at roughly the **midpoint of Apex->C**, and seed N by sliding from that M-seed **along the C->H direction far enough to reach the pinion's toe radius** (e.g. by the distance from the M-seed to A). Do NOT seed M/N just `Face Width` away from C/H — that starts N near H, far from its constraint target, and the solve fails to converge. Then apply **exactly these constraints** — all three are required, and the front face below is what holds N off the shaft axis, which is what the pre-Toe-Radius scheme used the A->Apex2 pin for:
