...
7. Draw the **spine** — a construction line from the local origin to the tooth-top point, defining
   ...
   sketch's geometry is fully constrained. The exact construction (sharing the endpoints, the +X horizontal
   reference and its required end-pin, built for every angle including 0, and the confirming angular dimension)
   is in `[SPUR-F-SPINE]`; the draw-and-confirm rule is `[SPUR-F-ROTATE-CONFIRM]`.
