...
- **Every sketch here is fully constrained** (`[PB-FULL-CONSTRAINT]`), with no exceptions. Where a
  sketch carries a point nothing else uses — the tooth generator's local origin in the Bore Profile
  sketch — ground it on the sketch's own `originPoint` rather than leaving it loose. See step 12.
