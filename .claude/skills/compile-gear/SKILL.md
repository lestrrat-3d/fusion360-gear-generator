...
Record the model identifier returned for the launched drafter in the timing `model` field. Use
that same resolved value for the spawn; do not run the resolver a second time for timing.
...
1. **Setup.** Work in a worktree, never the root checkout. Ensure `.tmp/` exists. Record the
   ...
   <the session's default model> [--mapping <json-path>]` and fix every
...
3. **Draft.** Spawn a subagent with the standard drafting prompt: run
   ...
   default model> [--mapping <the same json-path>]` and see
   `.claude/skills/generate-gear/MODELS.md` for the rule. Omit both mapping options when no
   mapping was supplied. Do
...
6. **Diagnose and loop.** Classify any failure with the table below. A draft fault goes back to
   ...
   role. Keep the same `--mapping` path on every fresh-spawn resolution when one was supplied.
   A continued round changes no model, because a resumed agent keeps the one it was spawned on.
   A fresh retry round re-renders the
