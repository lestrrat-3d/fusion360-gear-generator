...
class SpurGearCommandInputsConfigurator:
    ...
    def configure(cls, cmd):
        ...
        moduleInput = inputs.addValueInput('module', PARAM_MODULE, 'mm', adsk.core.ValueInput.createByReal(to_cm(1)))
...
def draw_spur_gear_circles(
    ...
    Returns:
        None (circles are drawn in the sketch)
    """
    curves = sketch.sketchCurves
    dimensions = sketch.sketchDimensions
    texts = sketch.sketchTexts
    ...
    # Store circles in state for use by draw_involute_tooth
    # We need to update the state object - but since dataclasses are immutable,
    # we'll just rely on the fact that the circles are in the sketch
    # draw_involute_tooth will find them by searching the sketch


def calculate_involute_point(base_circle_radius: float, intersection_radius: float) -> Optional[adsk.core.Point3D]:
    """
    ...
    Args:
        base_circle_radius: The radius of the base circle
    ...
    """
    if intersection_radius <= base_circle_radius:
        return None

    alpha = math.acos(base_circle_radius / intersection_radius)
    if alpha <= 0:
        return None
...
def draw_involute_tooth(
    ...
    characteristic involute curve shape of gear teeth by:
    1. Calculating involute curve points from base circle to tip circle
    2. Rotating the curve to center the tooth on the X axis
    3. Drawing mirrored left and right involute splines
    4. Adding the tooth tip arc
    5. Creating a central spine for constraints
    6. Adding ribs between the involutes for full constraint
    7. Connecting involutes to root circle if needed
    8. Optionally rotating the entire tooth by a specified angle

    Args:
        sketch: The sketch to draw in
        state: The current generation state (contains anchor_point)
        spec: The spur gear specification
    ...
    # Find the circles in the sketch by looking for circles with the expected radii
    # They were created by draw_spur_gear_circles
    root_circle = None
    base_circle = None
    pitch_circle = None
    tip_circle = None
    ...
    for circle in sketch.sketchCurves.sketchCircles:
        ...
        tolerance = 0.0001
        if abs(radius - spec.root_circle_radius) < tolerance:
            root_circle = circle
        elif abs(radius - spec.base_circle_radius) < tolerance:
            base_circle = circle
        elif abs(radius - spec.pitch_circle_radius) < tolerance:
            pitch_circle = circle
        elif abs(radius - spec.tip_circle_radius) < tolerance:
            tip_circle = circle

    if not all([root_circle, base_circle, pitch_circle, tip_circle]):
        raise Exception("Could not find all required circles in sketch - draw_spur_gear_circles must be called first")

    # Extract radii from spec
    tip_circle_radius = spec.tip_circle_radius
    base_circle_radius = spec.base_circle_radius
    pitch_circle_radius = spec.pitch_circle_radius
    root_circle_radius = spec.root_circle_radius

    # Calculate involute curve points
    # The involutes start from the base circle (or root if larger) and go to tip
    bigger_circle_radius = base_circle_radius

    involute_size = tip_circle_radius - bigger_circle_radius
    involute_steps = spec.involute_steps
    involute_points = []

    for i in range(involute_steps):
        intersection_radius = bigger_circle_radius + ((involute_size / (involute_steps - 1)) * i)
        involute_point = calculate_involute_point(bigger_circle_radius, intersection_radius)
        if involute_point is not None:
            involute_points.append(involute_point)

    # Calculate pitch point to determine rotation angle
    pitch_involute_point = calculate_involute_point(bigger_circle_radius, pitch_circle_radius)
    ...
    # Backlash (currently 0, but keeping the calculation for future use)
    backlash = 0
    backlash_angle = (backlash / pitch_circle_radius) * 0.25

    # Calculate rotation angle to center tooth on X axis
    rotate_angle = -((tooth_thickness_angle / 2) + pitch_point_angle - backlash_angle)

    # Rotate all involute points
    ...
    # Draw left involute spline
    point_collection = adsk.core.ObjectCollection.create()
    for point in involute_points:
        point_collection.add(point)
    left_line = curves.sketchFittedSplines.add(point_collection)

    # Draw right involute spline (mirrored)
    point_collection = adsk.core.ObjectCollection.create()
    for point in involute_points:
        point_collection.add(adsk.core.Point3D.create(point.x, -point.y, 0))
    right_line = curves.sketchFittedSplines.add(point_collection)

    # Draw the tooth tip arc
    tooth_top_point = points.add(adsk.core.Point3D.create(tip_circle_radius, 0, 0))
    constraints.addCoincident(tooth_top_point, tip_circle)
    top_arc = curves.sketchArcs.addByThreePoints(
        right_line.endSketchPoint,
        tooth_top_point.geometry,
        left_line.endSketchPoint
    )
    dimensions.addDiameterDimension(
        top_arc,
        adsk.core.Point3D.create(tooth_top_point.geometry.x, 0, 0)
    )
    ...
    # Handle angle constraint - port from OOP lines 306-320, 369-371
    ...
    if angle == 0:
        ...
        constraints.addCoincident(horizontal.endSketchPoint, tip_circle)
    ...
    for i in range(len(involute_points)):
        left_point = left_line.fitPoints.item(i)

        # Create rib connecting left and right involutes
        rib = curves.sketchLines.addByTwoPoints(
            left_point,
            right_line.fitPoints.item(i)
        )
        rib.isConstruction = True
    ...
    # Connect involutes to root circle if needed
    # If the first involute point is outside the root circle, we need connecting lines
    def draw_root_to_involute_line(root, root_radius, inv_line, x, y):
        ...
        line = curves.sketchLines.addByTwoPoints(point, inv_line.startSketchPoint)
        constraints.addCoincident(line.startSketchPoint, root)
        constraints.addHorizontal(line)
        return line
    ...
    tooth_profile_is_embedded = False

    if first_point_radius > root_circle_radius:
        # Involute extends beyond root circle, connect them
        draw_root_to_involute_line(root_circle, root_circle_radius, left_line, first_point.x, first_point.y)
        draw_root_to_involute_line(root_circle, root_circle_radius, right_line, first_point.x, -first_point.y)
    else:
        # Involute is embedded within root circle
        tooth_profile_is_embedded = True

    # Set the angle dimension value if we created one (OOP line 369-371)
    # This must be done AFTER all the lines have been drawn
    if angle != 0 and angle_dimension is not None:
        angle_dimension.value = angle

    # Update the state with tooth_profile_is_embedded flag
    # Since we can't modify the dataclass in place, we need to signal this somehow
    # The best approach is to update state.tooth_profile_is_embedded in the calling function
    # For now, we'll just set it in the state object if it's mutable
    # Actually, looking at GenerationState, tooth_profile_is_embedded has a default of False
    # We need to return this value somehow, but the design says return None
    # Let me check if we can modify the state object...
    # Since dataclasses are mutable by default (not frozen), we can update the field
    state.tooth_profile_is_embedded = tooth_profile_is_embedded
