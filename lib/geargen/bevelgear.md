...
Create a line through the projected center point. Apply **BOTH** `addCoincident(projectedCenter, anchorLine)` (the "intersection" — pins the center onto the line) **and** `addMidPoint(projectedCenter, anchorLine)` (center bisects the line). Use both, not midpoint alone. Give it a ~10mm length dimension (the value is arbitrary; it's only a reference line). **Stash the projected-center SketchPoint** (e.g. on `self`) so §2 re-projects *this* anchor-sketch point — not the raw user-selected center — into the Gear Profiles sketch. This line is the Anchor Line.
...
**Constrain the hexagon to the projected points — do NOT leave the edges free.** Build each edge sharing the projected SketchPoints (`addByTwoPoints(projected[i], projected[i+1])`), or build it from `projected[i].geometry` and then `addCoincident` each endpoint to its projected point — either way every hexagon vertex must be **pinned to a projected §2 point** (use exactly one binding method per the share-or-coincident rule, never both). This is load-bearing, not cosmetic: the hexagon's **first edge (A->G) is reused as the shaft axis for the revolve, pattern, bore plane, AND the meshing-rotation axis**, and the meshing rotation reads that edge's `startSketchPoint.worldGeometry` / `endSketchPoint.worldGeometry`. A **fully-unconstrained** SketchLine has an **undefined `worldGeometry`** (Fusion resolves its free endpoints against a default/world-XY frame), so a free hexagon edge silently builds the driving gear's meshing-rotation axis about the wrong line/origin and **moves the whole driving body onto world XY** (the pinion appears fine because it never reads the edge's `worldGeometry`). Pin the hexagon.
...
2. Then, cut the resulting pieces using the conical face **on the Gear Body (frustum)** generated from the C->H edge. Locate it the **same way — by the C->H edge's world MIDPOINT** (`(C_world + H_world)/2`): order the frustum's `ConeSurfaceType` faces by surface-distance to that midpoint and try each best-first as the split tool until one splits a piece (`isSplittingToolExtended=True`).

Apply the cuts **sequentially over the current set of pieces** (after cut 1, feed each resulting piece into cut 2). **The second cone may not intersect every piece** — wrap each split in a `try/except RuntimeError` and, when the message contains `SPLIT_TARGET_TOOL_NOT_INTERSECT` (or its localized form `交差`), keep that piece intact and continue rather than aborting. **Do NOT assume exactly three pieces.** With symmetric tooth counts the two cuts yield 3 pieces, but for asymmetric ratios the second (extended) cone can cross *both* cut-1 pieces, yielding **4 (or more)**. The next step selects the tooth robustly regardless of the count, so just require **at least 2 pieces** here (so at least one cut happened) and proceed; a bare "expected exactly 3" check is wrong and rejects valid asymmetric gears.
...
**Select the tooth robustly (works for 3, 4, or more pieces).** Remove **every** piece that contains the Apex — identify each by `BRepBody.pointContainment(apexWorld)` returning `PointInside` or `PointOn`. Of **all** the remaining (non-apex) pieces, keep the single **largest** by `BRepBody.physicalProperties.volume` (that is the tooth band) and remove **all** the others (the thin toe/heel stubs). Remove via `RemoveFeatures.add()` (so deletions appear in the timeline). This generalizes the symmetric case (3 pieces → remove apex, of 2 remaining keep the larger) to the asymmetric case (4+ pieces → remove apex, of 3+ remaining keep the largest); raise only if there is no non-apex piece at all.
...
2. Then, cut the resulting pieces using the cone face generated from the D->J edge (the analogue of C->H), located the same way by the **D->J edge midpoint** (`(D_world + J_world)/2`).

Apply the two cuts sequentially over the current pieces, catching `SPLIT_TARGET_TOOL_NOT_INTERSECT` / `交差` and keeping the un-split piece intact (exactly as for the pinion). **Do not assume exactly three pieces** (asymmetric ratios can yield 4+); require at least 2 and proceed.

Select the tooth robustly, exactly as for the pinion: remove **every** Apex-containing piece (`pointContainment` + `RemoveFeatures.add()`), then of all remaining non-apex pieces keep the single **largest** by volume (the tooth) and remove the rest. Raise only if no non-apex piece remains.
