...
**The helical rotation is driven, not constructed.** When a subclass passes a non-zero `angle` to the tooth generator, the geometry is still drawn at zero rotation — splines, ribs, and dimensions exactly as for a spur tooth. The tilt is applied as the very last step by setting the spine-to-horizontal angular dimension's value to `angle`, *after* the entire constraint network exists. Fusion then rotates the finished tooth as a rigid body. Trying to pre-rotate points leaves dimensions inconsistent with geometry and the sketch fails to solve.
...
- Constructor `(sketch, parent, angle=0)`. Store the angle (e.g. `self.toothAngle = angle`).
...
- `draw(anchorPoint, angle=0)` performs, in order: `drawCircles()`, `drawTooth(angle)`, then the
  **step-5 anchoring as its own final action** (see step 5). Methods `drawCircles`, `drawTooth`,
  `drawBore`, and `calculateInvolutePoint(baseRadius, intersectionRadius)` must all exist.
...
4. Rotate the (mirrored) sampled points by `rotate_angle`. This produces the **left** flank. Mirror that result across the X axis to produce the **right** flank.
...
6. Draw the **tooth-top arc**. This step is constraint-sensitive: add the *minimal* set below and **nothing more** — the extra constraints people reach for here (pinning the arc's centre, or separately coincident-constraining the arc endpoints) over-determine the sketch and it blows up later, when the last rib's perpendicular is added, with `VCS_SKETCH_OVER_CONSTRAINTS`.
   1. Materialize a **tooth-top point**: a `SketchPoint` at `(Tip Circle Radius, 0)`, constrained **coincident to the tip circle** — i.e. the *point* lies on the tip circle. (This is the only coincidence here. Do **not** constrain the arc's `centerSketchPoint` to anything — no concentric/centre-on-origin constraint.)
...
7. Draw the **spine**: a construction line created as `addByTwoPoints(localOrigin, toothTopPoint)` — pass **both** existing `SketchPoint`s directly (the local-origin point as the start, the step-6 tooth-top point as the far end), so the spine *shares* them. Do **not** create it from `.geometry` coordinates, do **not** add a separate start-coincident to the origin (sharing already ties it; an extra coincident here is what makes the solver fail), and do **not** constrain the spine's end onto the arc (the tooth-top point already lies on the tip circle). For `angle = 0` (spur), the spine's only additional constraint is **horizontal**. Otherwise add a horizontal reference construction line from the origin and an angular dimension between the spine and that reference — measured **from the spine to the horizontal reference, in that argument order** (`addAngularDimension(spine, horizontal, …)`); the order fixes the sign of the tilt, which sets the helix hand for the subclasses. Leave the dimension's value unset for now — it becomes the driver that tilts the finished tooth (see Sketch Discipline); set its value to `angle` as the very last action, after the whole tooth is constrained.
8. Draw a **rib** construction line between each pair of matching left/right flank points — passing through the splines' `fitPoints[i]` so the ribs share endpoints with the splines. Each rib needs a materialized **midpoint sketch point** on the spine, because a rib's midpoint isn't a referenceable entity you can dimension to unless you create a point for it. Build each rib like this, **in this exact order** — the order matters, a different order over-constrains the sketch (Fusion raises `VCS_SKETCH_OVER_CONSTRAINTS`):
   ...
   3. Add a fresh `SketchPoint` for the midpoint, created **already on the spine** — at coordinates `(fitPoints[i].x, 0, 0)` (the spine is horizontal at draw time, before any tilt, so its line is `y = 0` through the local origin). Do **not** create it at the rib's true 2-D midpoint `((xL+xR)/2, (yL+yR)/2)` — starting it off the spine and then constraining it on is what triggers the over-constraint.
